...
export const loadCampaigns = async (
  path: string,
  qs: string,
): Promise<Campaign[] | ResponseError | null> => {
  const url = path + "?" + qs;
  const response =
...
export const updateCampaign = async (
  ...
  if (campaign.language === "")
    throw new Error(
      'You must select a language. If it is wikimedia Commons, select "commons"',
    );

  const payload = {
    ...campaign,
    coordinators: campaign.coordinators.map((c) =>
      typeof c === "string" ? c : c.username,
    ),
  };
  const res = await fetchAPIFromBackendSingleWithErrorHandling<Campaign>(
...
export const updateCampaignStatus = async (
  campaignId: string,
  isArchived: boolean,
): Promise<ResponseSingle<Campaign> | ResponseError> => {
  return fetchAPIFromBackendSingleWithErrorHandling<Campaign>(
...
export const updateRoundStatus = async (
  ...
  return fetchAPIFromBackendSingleWithErrorHandling(
...
export const deleteRound = async (roundId: string) => {
  return fetchAPIFromBackendSingleWithErrorHandling<{ roundId: string }>(
    `/round/${roundId}`,
    {
      method: "DELETE",
    },
...
export const addMyselfAsPublicJury = async (roundId: string) => {
  return fetchAPIFromBackendSingleWithErrorHandling(
    `/round/${roundId}/addMyselfAsPublicJury`,
    {
      method: "POST",
    },
  );
